...
object Melody {
  ...
  def apply(numBars: Int, startingTick: Int, range:Int): Melody = {
    ...
    val ticks = timeSpots.dropRight(1)

    //take the pitches, rhythms, and ticks lists, iterate over them together
  ...
  def combine(melodies: List[Melody]): Melody = {
    Melody(melodies.foldLeft(List[Note]()){ (acc, melody) =>
      acc ::: Transformers.shiftInTime(
        melody,
        acc.lastOption.map(n => n.tick + n.length).getOrElse(0)
      ).notes
